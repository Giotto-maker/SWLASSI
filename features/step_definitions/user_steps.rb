Given /^a registered user$/ do
  ...
  User.create!(:email => ENV['TEST_USER_EMAIL'], :name => ENV['TEST_USER_NAME'], :surname => ENV['TEST_USER_SURNAME'],
...
Given /^another user with base privileges$/ do
  password = ENV['TEST_USER_PSW']
  User.create!(:email => "ferrazzivalentina@gmail.com", :name => ENV['TEST_USER_NAME'], :surname => ENV['TEST_USER_SURNAME'],
  :username => ENV['TEST_USER_SURNAME'] , :category => 'Architecture', :roles_mask => 0, :password => password, 
  :password_confirmation => password, confirmed_at: Time.now.utc)
end

Given /^a logged in base user$/ do
  visit new_user_session_path
  fill_in "Email", :with => "ferrazzivalentina@gmail.com"
