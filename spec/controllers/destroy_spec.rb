...
RSpec.describe ArtworksController do
    ...
    it "should NOT destroy the artwork if the user is a base user" do
        sign_in @test_user3
        get :destroy, params: {id: @colosseum.id}

        expect(response.status) == 403
        expect(response.body) == 'redirected'
    end

    it "should NOT destroy the artwork if the user is an artolover user" do
        sign_in @test_user2
        get :destroy, params: {id: @colosseum.id}
    ...
    it "should destroy the artwork if the user is an admin user and create a deletion entry" do
        sign_in @test_user1
        get :destroy, params: {id: @colosseum.id}
        del = Deletion.all

        expect(response.status) == 200
        expect(response.body) == 'Artwork deleted'

        expect(del[0]).to_not be_nil
        expect(del[0].user_id) == @test_user1.id
        expect(del[0].artwork_id) == @colosseum.id
        
        expect(assigns(:artwork).destroyed?).to be true
    end
